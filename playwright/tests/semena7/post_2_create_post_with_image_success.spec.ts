...
test('create_post_with_image_success', async ({ page }) => {
  ...
  await page.screenshot({path: '../../result/post_1_create_succesful/1-dashboard__webkit.png'})
  ...
  await page.getByPlaceholder('Search free high-resolution photos').click();
  await page.getByPlaceholder('Search free high-resolution photos').fill('blue');

  
  await page.getByRole('link', { name: 'Insert image' }).first()?.click();

  await page.waitForTimeout(5000);

  await expect(page).toHaveURL(urlText);
  let image = await page.getByRole('img');
  ...
  await page.screenshot({path: '../../result/post_2_create_post_with_image_success/2-post_screen_before_save__webkit.png'})
  await page.getByText('Draft - Saved').click();
  await page.goto(`${_baseUrlGhost}#/dashboard`);
  await page.screenshot({path: '../../result/post_2_create_post_with_image_success/3-post_screen_after_save__webkit.png'})
});